...
def get_pubchem_cid(smiles: str) -> int | None:
    ...
    try:
        ...
        st.error(f"PubChem HTTP error (SMILES→CID): {http_err}")
        return None
...
def get_ecosar_data(cid: int) -> dict | None:
    ...
    try:
        ...
        return {
            ...
            "Fish LC₅₀ (mg/L)": to_float(first.get("Fish_LC50_mg_L", None)),
            "Daphnia EC₅₀ (mg/L)": to_float(first.get("Daphnia_EC50_mg_L", None)),
            "Algae EC₅₀ (mg/L)": to_float(first.get("Algae_EC50_mg_L", None)),
        }
    except requests.exceptions.HTTPError as http_err:
        st.error(f"PubChem HTTP error (ECOSAR): {http_err}")
        return None
    except Exception as e:
        st.error(f"Error fetching ECOSAR data from PubChem: {e}")
        return None

def calculate_risk_score(props: dict, ecosar: dict) -> int:
    score = 0
    logp = props.get("logP")
    ...
    if daph_ec50 is not None and daph_ec50 <= 1.0:
        score += 20

    algae_ec50 = ecosar.get("Algae EC₅₀ (mg/L)") if ecosar else None
    if algae_ec50 is not None and algae_ec50 <= 1.0:
        score += 20
